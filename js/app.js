...
if(avatarUrlCache[cleanName]){
return avatarUrlCache[cleanName];
}
...
document.getElementById(pos+"_status").innerHTML =
`<span class="status idle"></span>`;
...
console.log("NO AVATAR URL:",username);
...
let img =
await loadImage(avatarURL,5);

if(!img){

console.log("LOAD FAIL:",avatarURL);
...
Object.keys(avatarUrlCache).forEach(k=>{
delete avatarUrlCache[k];
});

positions.forEach(pos=>{

document.getElementById(pos.id+"_status").innerHTML =
`<span class="status loading"></span>`;
